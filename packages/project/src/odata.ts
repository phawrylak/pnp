import { ProjectQueryableConstructor } from "./projectqueryable";
...
class ProjectODataEntityParserImpl<T, D> extends ODataParserBase<T & D> {

    constructor(protected factory: ProjectQueryableConstructor<T>) {
        super();
    }
...
class ProjectODataEntityArrayParserImpl<T, D> extends ODataParserBase<(T & D)[]> {
...
export function projectODataEntity<T, DataType = any>(factory: ProjectQueryableConstructor<T>): ODataParser<T & DataType> {
    return new ProjectODataEntityParserImpl<T, DataType>(factory);
}

export function projectODataEntityArray<T, DataType = any>(factory: ProjectQueryableConstructor<T>): ODataParser<(T & DataType)[]> {
    return new ProjectODataEntityArrayParserImpl<T, DataType>(factory);
}
